...
SC_MODULE(Target) {
    ...
    SC_CTOR(Target):socket("socket"), n_trans(0), response_in_progress(false), next_response_pending(0), end_req_pending(), m_peq(this, &Target::peq_cb) {
                ...
                if (phase == internal_ph) {
                    ...
                    if (cmd == tlm::TLM_READ_COMMAND) {
                        *reinterpret_cast<int*>(ptr) = rand();
                        fout<<hex<<adr<<" "<<name()<<" Execute READ, target = "<<name()<<" data = "<<*reinterpret_cast<int*>(ptr)<<endl;
                    }
                    else if (cmd == tlm::TLM_WRITE_COMMAND) {
                        fout<<hex<<adr<<" "<<name()<<" Execute WRITE, target = "<<name()<<" data = "<<*reinterpret_cast<int*>(ptr)<<endl;
                    }
